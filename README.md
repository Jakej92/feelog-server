...
▶ 공식 채널
- 공식 채널 정보 출력 및 채널 구독 기능
- 공지/챌린지 목록 출력
- 공지 목록 페이징 처리

▶ 메인페이지 헤더
- 작성하기(감정 일기, 포스트, 응원글)
- 감정 일기 작성 후 AI가 스코어로 평가, 평가 후 각 점수에 해당하는 이미지, 멘트 출력 / 클라이언트가 직접 스코어 선택 가능
- 감정 일기 공개 여부, 닉네임 공개 여부, 태그 기능(공개/구독자에게만 공개/비공개, 익명/닉네임)
- 감정 일기 작성 폼 JQuery SummerNote 사용, 글꼴/이미지 첨부 기능 포함
- 포스트 작성 Jquery SummerNote 사용, 글꼴/이미지 첨부 기능 포함
- 포스트 작성 후 미리보기 화면, 태그, 포스트 타입(포스트/응원글) 기능
- 검색창에 키워드 입력 시, 감정일기 / 포스트 / 응원글 / 채널 항목별로 분류된 검색 결과 출력
- 각 항목은 최대 5개까지 초기에 표시되며, "더보기" 버튼 클릭 시 추가 로딩
- 감정일기와 포스트는 썸네일, 제목, 내용 일부, 작성자 프로필 및 태그 정보 함께 표시
- 검색 결과 클릭 시 해당 상세 페이지로 이동
- 최근 검색 기록, 인기 태그, 연관 검색어 등 추가 노출 가능
- 검색창 입력값은 엔터 또는 검색 버튼 클릭으로 트리거됨
